...
- `make fetch-runc` — downloads the pinned static `runc` v1.5.1 release binary for the host arch into `bin/runc-<arch>`, verifying its sha256 against the release's PGP-signed `runc.sha256sum` manifest. **Opt-in**: when that file is present, `make init` stages it into the initfs at `/sbin/runc`, and a container can select the runc-backed launch path with `ociRuntimePath = "/sbin/runc"` (`cctl run --oci-runtime-path /sbin/runc`). Without it the initfs is byte-identical to before and `vmexec` is the only runtime, so `make all` performs no new network access. The `runc *` integration tests skip themselves when it hasn't been fetched. Unlike `fetch-cloud-hypervisor`, this downloads to a `.tmp` path and only moves it into place after the checksum passes — the staging decision downstream is presence-only, so a truncated binary from a failed download would otherwise be baked into the guest image silently.
- `make build-cloud-hypervisor` / `make build-virtiofsd` — build patched `cloud-hypervisor` / `virtiofsd` from sources in `.local/cloud-hypervisor` and `.local/virtiofsd`. Each target depends on `make fetch-cloud-hypervisor-source` / `make fetch-virtiofsd`, which do a full-history clone at the pinned tag (`CLOUD_HYPERVISOR_VERSION`, `VIRTIOFSD_VERSION` in the Makefile) when the directory is absent, and leave an existing checkout untouched — check out a different revision yourself to build it. `build-virtiofsd` applies `scripts/patches/virtiofsd-skip-cap-drop-with-sandbox-none.patch` and is idempotent. Both run inside the same Linux dev container as `linux-integration` so the resulting binaries are aarch64-linux-gnu.
- `make dist-x86_64` — assembles `bin/containerization-x86_64-<sha>.tar.gz` (cctl + cloud-hypervisor + virtiofsd + initfs.ext4 + kernel) for x86_64 Linux deployment, cross-compiled inside the aarch64 dev container via the Static Linux SDK (Swift) and `cargo zigbuild` (Rust). Prereqs: `.local/cloud-hypervisor` and `.local/virtiofsd` source checkouts (fetched automatically, as above), and an x86_64 kernel built via `make -C kernel TARGET_ARCH=x86_64`. Per-stage rebuild env vars: `REBUILD_VMINITD=1`, `REBUILD_INITFS=1`, `REBUILD_CH=1`, `REBUILD_VIRTIOFSD=1`; cctl x86 always rebuilds. **Full pipeline, toolchain rationale, and troubleshooting in `docs/x86_64-build.md`.** The orchestrator is `scripts/build-dist-x86_64.sh`.
...
- `ManagedContainer.swift` / `ManagedProcess.swift` launch container processes via `vmexec` by default. Supplying an OCI runtime path switches both the init process (`RuncProcess.swift`) and execs (`RuncExecProcess.swift`) to `runc`, driven by the client in `VminitdCore/Runc/`. `ProcessSupervisor` reaps and dispatches exit events for both paths.
