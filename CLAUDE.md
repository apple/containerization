...
- `VminitdCore/Runc/` plus `RuncProcess.swift` / `ManagedContainer.swift` / `ManagedProcess.swift` shell out to `runc` for actual container execution. `ProcessSupervisor` reaps and dispatches exit events.
