...
extension Application {
    ...
    struct Run: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            // cctl owns <appRoot>/containers/<id> for the run and tears it down
            // in the defer above. Ctrl-C and SIGKILL both skip that defer, so
            // reclaim a root left by an earlier run rather than letting
            // ContainerManager reject the id as a duplicate. Idempotent when
