...
public struct ContainerManager: Sendable {
    ...
    public mutating func create(
        ...
        // The default boot log lives under the container root. The image-based
        // overloads create that directory; a caller-supplied rootfs reaches
        // here without it, and both backends fail to open the log if it is
        // missing.
        let stateDirectory = containerRoot.appendingPathComponent(id)
        ...
        return try LinuxContainer(
            ...
            config.bootLog = BootLog.file(path: stateDirectory.appendingPathComponent("bootlog.log"))
