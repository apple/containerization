...
struct VsockTransportTests {
    ...
    @Test func unixSocketRelayStopKeepsGuestConnectionAliveUntilRelayFinishes() async throws {
        let (relayFd, peerFd) = try makeSocketPair()
        defer { close(peerFd) }

        let counter = CloseCounter()
        let queue = DispatchQueue(label: "com.apple.containerization.tests.unix-socket-relay.stop")
        let socketPath = uniqueSocketPath()
        defer {
            try? FileManager.default.removeItem(atPath: (socketPath as NSString).deletingLastPathComponent)
        }
        ...
        #expect(fcntl(relayFd, F_GETFD) != -1)
